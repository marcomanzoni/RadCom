...
fs                  = 4*B;     % Sampling frequency in fast time [Hz];
...
signal_tx = signal_tx_bb;%.*exp(1j*2*pi*f0*t(:));
...
for kk = 1:Nsymbols % symbols = slow time samples
    for ii = 1:N_ant % Antennae
        for n = 1:Nt % Targets
            ...
            s = sqrt(Prx)*interp1(t', signal_tx(:,kk), t'-tau, "cubic", 0).*exp(-1j*2*pi*f0*tau);
...
for kk = 1:Nsymbols % symbols = slow time samples
    ...
    fprintf("Focusing slow-time %d/%d: ", kk, Nsymbols);
    ...
    for ii = 1:N_ant
        ...
        I(:,:,kk) = I(:,:,kk) + interp1(t, Drc(:, ii, kk) , tau,"linear",NaN).*exp(+1j.*2*pi*f0*tau);
        I_n(:,:,kk) = I_n(:,:,kk) + interp1(t, Drc_n(:,ii,kk), tau,"linear",NaN).*exp(+1j.*2*pi*f0*tau);
    end
    fprintf("Done. \n");
end
...
figure; imagesc(x_foc, y_foc, abs(I_SAR)); axis xy tight; title("SAR image without noise");
xlabel("x [m]"); ylabel("y [m]"); colorbar; colormap(lajolla);

figure; imagesc(x_foc, y_foc, abs(I_SAR_n)); axis xy tight; title("SAR image with noise");
xlabel("x [m]"); ylabel("y [m]"); colorbar; colormap(lajolla);
